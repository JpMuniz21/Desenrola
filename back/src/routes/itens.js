...
router.get('/simular-caucao/:id', async (req, res) => {
    ...
    try {
        const result = await connection.query(
            'SELECT id_item, preco FROM item WHERE id_item = $1', [id]
        );
    ...
    try {
        const result = await connection.query(
            `INSERT INTO item (nome, descricao, id_usuario, id_categoria, preco, periodo, imagem, status) 
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id_item, nome, status`,
        ...
        await connection.query(
            `UPDATE item 
             SET nome = COALESCE($1, nome), descricao = COALESCE($2, descricao), 
                 id_categoria = COALESCE($3, id_categoria), preco = COALESCE($4, preco), 
                 periodo = COALESCE($5, periodo), imagem = COALESCE($6, imagem), status = COALESCE($7, status)
             WHERE id_item = $8`,
